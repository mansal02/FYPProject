...
class AgentWorker(QThread):
    ...
    def run(self) -> None:
        try:
            # FIX: Use a defensive attribute fallback to prevent unexpected crashes
            intent = "general"
...
class AssistantMainWindow(QMainWindow):
    def __init__(self, db: DatabaseManager, user_id: int, username: str) -> None:
        ...
        if self.live2d_enabled:
            ...
            self._set_avatar_status("Avatar disabled.")
    ...
    def _build_chat_tab(self) -> None:
        ...
        # New Integrated Minimize Control Bar directly on the Avatar Holder
        ...
        self.minimize_avatar_btn.setStyleSheet("""
            QPushButton {
                background: #ff4d6d; color: #0b0f16; border: 1px solid #6f1d2c; 
                border-radius: 6px; font-weight: bold; font-size: 14px; padding: 0px;
            }
        ...
        face_layout.addLayout(avatar_control_bar)
        ...
        # Floating Context recovery button if panel is closed out
    ...
    def _build_rad_tab(self) -> None:
        ...
        self.rad_category_combo.addItems(["user_fact", "preference", "task"])
        ...
        row = QHBoxLayout()
        refresh_btn = QPushButton("Sync Data Frame")
    ...
    def on_stop_clicked(self) -> None:
        ...
        Avoids calling .terminate() to protect database writing layouts from memory corruption.
        """
        self.agent.stop()
    ...
    def on_save_preferences_clicked(self) -> None:
        """
        Unified preferences handler. Synchronizes active checkbox elements 
        across tabs and updates both runtime agents and underlying SQLite storage.
        """
    ...
    def _load_rad_table(self) -> None:
        ...
        for idx, row in enumerate(self.db.get_rad_data(self.current_user_id, limit=100)):
...
def main() -> None:
    ...
    window = AssistantMainWindow(db=db, user_id=int(auth_data.get("user_id", 0)), username=str(auth_data.get("username", "user")))
    window.show()
    sys.exit(app.exec_())
