...
class MarieMemoryAgent:
    """
    Manages embedding and storing memory files into ChromaDB using Ollama.
    """
    def __init__(self) -> None:
        ...
        self.default_top_k = int(cfg.get("top_k", 4)) # Fixed: Added missing default attribute
    ...
    def _iter_files(self) -> List[Path]:
        ...
        return [
            ...
            if p.is_file() and p.suffix.lower() in self.watch_extensions
        ]
    ...
    def _chunk(self, text: str) -> List[str]:
        ...
        while start < len(clean):
            ...
            if piece := clean[start:end].strip():
                parts.append(piece)
            if end >= len(clean):
                break
    ...
    def _extract_embeddings(payload) -> List[List[float]]:
        ...
        if hasattr(payload, "model_dump"):
            try: payload = payload.model_dump()
    ...
    def _get_embedding_payload(self, text_or_list):
        """Unified embedding call accommodating Ollama API variations."""
            ...
            if hasattr(client, method_name):
                try:
                    ...
                    if method_name == "embed":
                        kwargs["input"] = text_or_list
                    else:
                        kwargs["prompt"] = text_or_list
    ...
    def _embed_batch(self, texts: Sequence[str]) -> List[List[float]]:
        """Batches embeddings with a one-by-one fallback."""
    ...
    def process_file(self, file_path: Path) -> bool:
        if self._collection is None: return False
        ...
        try: signature = self._signature(path_obj)
        ...
        try: text = path_obj.read_text(encoding="utf-8", errors="ignore")
        ...
        with self._lock:
            ...
            if len(embeddings) != len(chunks): return False

            ids, docs, metas = [], [], []
            try: rel_source = str(path_obj.relative_to(self.watch_dir))
            except Exception: rel_source = source

            for idx, chunk in enumerate(chunks):
                ids.append(self._make_chunk_id(source, idx, chunk))
                docs.append(chunk)
                metas.append({"source": source, "relative_source": rel_source, "chunk": idx})
    ...
    def refresh_all(self) -> int:
        """Processes all files in the watch directory."""
        return sum(1 for path in self._iter_files() if self.process_file(path))

    def query(self, question: str, top_k: Optional[int] = None) -> str:
        """Queries ChromaDB based on embedding similarity."""
        if self._collection is None: return ""
        ...
        for idx, doc in enumerate(docs):
            if snippet := self._collapse_ws(str(doc)):
                source = "memory"
                if idx < len(metas) and isinstance(metas[idx], dict):
                    source = str(metas[idx].get("relative_source") or metas[idx].get("source") or source)
...
def get_memory_agent_context(question: str, top_k: Optional[int] = None) -> str:
    ...
    return agent.query(question, top_k=top_k) if agent.is_ready else ""
...
class _MemoryEventHandler(FileSystemEventHandler):
    ...
    def _try_process(self, path_text: str) -> None:
        try: self.agent.process_file(Path(path_text))
        except Exception: pass
    ...
    def on_deleted(self, event) -> None:
        if not getattr(event, "is_directory", False):
            try: self.agent.remove_file(Path(str(event.src_path)))
            except Exception: pass
...
def run_memory_agent_watcher() -> None:
    ...
    if chromadb is None or ollama is None:
        print("[MEMORY] Missing dependency: chromadb or ollama")
        return
    ...
    observer = Observer()
    observer.schedule(_MemoryEventHandler(agent), str(agent.watch_dir), recursive=True)
    observer.start()
