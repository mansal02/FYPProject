...
from aiassistant.tools.tools_os import create_outlook_meeting, extract_document_text
...
from aiassistant.workers.worker import (
    OfflineLLMClient, 
    OfflineWorker, 
    OnlineLLMClient, 
    OnlineWorker
...
SYSTEM_PROMPT = (
    "You are an offline desktop assistant. Prioritize action over explanation.\n"
    ...
    "For calendar events, use the tool format:\n"
    "<tool>{\"action\": \"create_outlook_meeting\", \"subject\": \"Project Sync\", \"start\": \"2026-06-12T10:00:00\", \"duration_minutes\": 30, \"attendees\": [\"partner@example.com\"]}</tool>\n\n"
    "To analyze a document, use the tool format:\n"
    "<tool>{\"action\": \"analyze_document\", \"file_path\": \"C:/docs/report.pdf\", \"instruction\": \"Extract the key financial metrics.\"}</tool>"
)
...
class AgentFactory:
    ...
    def create_workers(self, mode: str) -> Dict[str, Any]:
        ...
        for spec in specs:
            workers[spec.intent] = (
                OfflineWorker(spec, spec.offline_pipeline, self.db, self.config) 
                if mode == "offline" else 
                OnlineWorker(spec, spec.hybrid_pipeline, self.db, self.config)
            )
        return workers
    ...
    name="Office Worker", 
    intent="office", 
    description="Handles Excel, Word, PPT, document analysis, and calendar meetings.", 
    tools={
        "create_word_doc": lambda **kwargs: run_tool_action({"action": "create_word_doc", **kwargs}),
        "create_excel_report": lambda **kwargs: run_tool_action({"action": "create_excel_report", **kwargs}),
        "create_outlook_meeting": create_outlook_meeting,
        "analyze_document": extract_document_text
    }, 
    offline_pipeline=shared_pipeline, 
    hybrid_pipeline=make_hybrid(True)
),
            WorkerSpec(
    name="Communication Worker", 
    intent="communication", 
    description="Operates messaging, emails, and attachments.", 
    tools={
        "send_email": lambda **kwargs: run_tool_action({"action": "send_email", **kwargs}),
        "send_whatsapp": lambda **kwargs: run_tool_action({"action": "send_whatsapp", **kwargs}),
        "send_telegram": lambda **kwargs: run_tool_action({"action": "send_telegram", **kwargs})
    }, 
    offline_pipeline=shared_pipeline, 
    hybrid_pipeline=make_hybrid(False)
),
...
class OfflineAgentCore:
    ...
    def process_user_message(self, user_text: str) -> str:
        ...
        action_handler = ActionHandler(db=self.db)
        
        if intent in ["office", "os", "web"] or action_handler._looks_like_action_command(text):
            try:
                print(f"[Short-Circuit Pipeline] Bypassing LLM loop for intent: {intent}")
                output_message = action_handler.execute_and_collect(text)
                if output_message:
                    final_reply = self.clean_output_for_ui(output_message)
                    self.db.log_interaction(self.session_id, role="assistant", message=final_reply, category="chat")
                    return final_reply
            except Exception as route_err:
                print(f"[Short-Circuit Warning] Fast route failed, falling back to LLM: {route_err}")    
    ...
    def _reason_with_external(self, messages: List[Dict[str, str]]) -> str:
        ...
        payload = {
            ...
            "generationConfig": {"temperature": self.config.temperature, "maxOutputTokens": 640}
        }
