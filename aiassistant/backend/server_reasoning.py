...
EXTERNAL_MODEL = str(CONFIG.get("runtime", {}).get("external_model", "gemini-2.5-flash")).strip()
...
_SENTIMENT_TAGS = {
    ...
    "neutral", "concerned", "friendly", "serious", "annoyed", "bored",
}
_SENTIMENT_KEYWORDS = {
...
def _gemini_generate_text(prompt: str, temperature: float) -> str:
    ...
    payload = {
        ...
        "generationConfig": {"temperature": float(temperature), "maxOutputTokens": 640},
    }
    try:
...
def _auto_extract_memory_async(user_text: str):
    """Runs a fast background LLM pass to extract facts and save to DB."""
    if not user_text or len(user_text.strip()) < 5:
        return  # Skip very short generic messages
        
    extraction_prompt = (
        "Extract any permanent facts, user preferences, or ongoing tasks from this message. "
        "Output ONLY a valid JSON dictionary with keys 'category' (fact, preference, or task), "
        "'key' (a short 1-3 word summary), and 'value'. If there is nothing to save, output empty JSON {}."
        f"\nMessage: {user_text}"
    )
    
    try:
        # 1. FIX: Instantiate a local DB connection strictly for this thread
        from aiassistant.infra.db.database_manager import DatabaseManager
        local_db = DatabaseManager()
        
        response = ollama.chat(
            model=CONFIG.get("ollama", {}).get("fast_model", "qwen2.5:3b"), 
        ...
        # 2. FIX: Use Regex to aggressively strip out conversational filler
        import re
        json_match = re.search(r'\{.*\}', result_text, re.DOTALL)
        
        if not json_match:
            print(f"[MEMORY WARN] Model failed to output JSON. Raw output: {result_text}")
            return
            
        data = json.loads(json_match.group(0))
        
        # If it's not an empty dictionary, save it
        if data and "category" in data and "key" in data and "value" in data:
            if hasattr(local_db, "add_rad_data_if_new"):
                local_db.add_rad_data_if_new(
                    category=str(data["category"]).lower(),
                    key=str(data["key"]),
                    value=str(data["value"]),
                    confidence=0.9
                )
                print(f"[MEMORY SUCCESS] Saved to Database -> {data['category']}: {data['key']}")
            else:
                print("[MEMORY ERROR] add_rad_data_if_new method missing in DatabaseManager.")
                
    except Exception as e:
        # 3. FIX: Expose the error so we can actually see why it fails
        print(f"[MEMORY THREAD CRASH] {str(e)}")
...
def get_marie_response_stream(prompt, memory_context="", online_mode="auto"):
    ...
    try:
        ...
        # Safely fetch style profile if the DB method exists
        try:
            style_profile = db.get_style_profile("train_root") if hasattr(db, "get_style_profile") else None
        except Exception:
            style_profile = None
        ...
        if memory_context:
            ...
            system_instructions += (
                f"\n[CRITICAL CONTEXT] Incorporate the following retrieved memories into your response implicitly:\n"
                f"{recent_facts}\n"
            )
...
def _resolve_request_context(payload):
    ...
    if not rag_context:
        rag_context = db.get_all_rad_data() if hasattr(db, "get_all_rad_data") else ""
...
def chat_endpoint(payload: dict = Body(...)):
    ...
    if user_text:
        threading.Thread(target=_auto_extract_memory_async, args=(user_text,), daemon=True).start()
        
    online_mode = _resolve_online_mode(payload)
    crew_config = CONFIG.get("crew", {})
    
    # Run background memory extraction for the user's message
...
def chat_multi_agent_endpoint(payload: dict = Body(...)):
    ...
    try:
        ...
        return {"researcher": "", "coder": "", "final": f"Multi-agent mode failed: {e}"}
...
def chat_stream_endpoint(payload: dict = Body(...)):
    ...
    if _should_use_crew(user_text, crew_config):
        ...
        def generate_crew_events():
            if not summary:
                yield json.dumps({"type": "error", "content": str(crew_result.get("error") or "Crew processing failed."), "request_id": request_id}) + "\n"
                return

            for chunk in _chunk_text(summary):
                yield json.dumps({"type": "token", "content": chunk}) + "\n"

            yield json.dumps({"type": "done", "full_response": summary, "sentiment": _extract_sentiment_tag(summary), "request_id": request_id, "cancelled": False}) + "\n"
